"""
PDF export helpers for review bundles.
"""
...
def _format_top_probabilities(case_packet: dict) -> str:
    ...
    lines.extend(f"- {item['label']}: {float(item['probability']):.3f}" for item in items)
    return "\n".join(lines)
...
def _render_annotated_image(fig: plt.Figure, image, detections: list[dict], bounds: list[float]) -> None:
    ...
    for index, detection in enumerate(detections[:5]):
        ...
        ax.add_patch(Rectangle((x1, y1), x2 - x1, y2 - y1, fill=False, linewidth=2.0, edgecolor=color))
        ax.text(
            x1,
            max(4, y1 - 6),
...
def _format_support_lines(payload: dict) -> list[str]:
    ...
    review = payload["review"]
    lines = [f"- {label}: {case_packet['swin']['probabilities'].get(label, 0.0):.2f}" for label in review["supported_findings"]]
...
def _format_uncertain_lines(payload: dict) -> list[str]:
    case_packet = payload["case_packet"]
    review = payload["review"]
    lines = [f"- {label}: {case_packet['swin']['probabilities'].get(label, 0.0):.2f}" for label in review["uncertain_findings"]]
    return lines or ["- None"]
...
def _draw_text_block(
    ...
    for line in lines:
        if not line.strip():
            ...
            continue
        if line.startswith("- "):
            chunks = textwrap.wrap(line[2:], width=width - 2, break_long_words=False, break_on_hyphens=False)
            wrapped.append(f"- {chunks[0]}" if chunks else "-")
    ...
    fig.text(x, y, title, fontsize=HEADING_FONTSIZE, fontweight="bold", color=HEADER_COLOR, ha="left", va="top")
    fig.text(x, y - 0.03, "\n".join(wrapped), fontsize=font_size, ha="left", va="top", color=TEXT_COLOR)
...
def _draw_first_page(pdf: PdfPages, payload: dict) -> None:
    ...
    fig.text(LEFT_MARGIN, TOP_MARGIN, "AI Decision-Support Report", fontsize=TITLE_FONTSIZE, fontweight="bold", ha="left", va="top", color=HEADER_COLOR)
    fig.text(LEFT_MARGIN, TOP_MARGIN - 0.04, f"Case: {payload['image_id']}    Source image: {image_path.name}", fontsize=SUBTITLE_FONTSIZE, ha="left", va="top", color=TEXT_COLOR)
    _render_original_image(fig, image, [0.08, 0.57, 0.36, 0.26])
    _render_annotated_image(fig, image, detections, [0.56, 0.57, 0.36, 0.26])
    _draw_text_block(fig, "At A Glance", x=0.08, y=0.47, width=110, max_lines=None, font_size=9.0, lines=_format_at_a_glance(payload))
    pdf.savefig(fig)
    plt.close(fig)
...
def _draw_clinical_summary_page(pdf: PdfPages, payload: dict) -> None:
    ...
    fig.patch.set_facecolor("white")
    review = payload["review"]
    fig.text(LEFT_MARGIN, TOP_MARGIN, "Clinical Summary", fontsize=TITLE_FONTSIZE, fontweight="bold", ha="left", va="top", color=HEADER_COLOR)
    _draw_text_block(fig, "Findings", [review["findings_section"]], x=0.08, y=0.88, width=105, max_lines=None, font_size=10.0)
    _draw_text_block(fig, "Impression", [review["impression_section"]], x=0.08, y=0.58, width=105, max_lines=None, font_size=10.0)
    guidance_lines = [
    ...
    _draw_text_block(fig, "Review Guidance", guidance_lines, x=0.08, y=0.30, width=105, max_lines=None, font_size=9.6)
    fig.text(LEFT_MARGIN, 0.08, "\n".join(textwrap.wrap(review["safety_note"], width=120, break_long_words=False, break_on_hyphens=False)), fontsize=SMALL_FONTSIZE, ha="left", va="top", color=TEXT_COLOR)
    pdf.savefig(fig)
    plt.close(fig)
...
def _draw_evidence_page(pdf: PdfPages, payload: dict) -> None:
    ...
    image = mpimg.imread(Path(payload["image_path"]))
    detections = sorted(payload["case_packet"]["yolo"]["detections"], key=lambda item: float(item["confidence"]), reverse=True)
    fig.text(LEFT_MARGIN, TOP_MARGIN, "Supporting Evidence", fontsize=TITLE_FONTSIZE, fontweight="bold", ha="left", va="top", color=HEADER_COLOR)
    _render_annotated_image(fig, image, detections, [0.08, 0.48, 0.52, 0.38])
    _draw_text_block(fig, "YOLO Detections", _format_yolo_legend(detections), x=0.64, y=0.84, width=34, max_lines=None, font_size=9.0)
    _draw_text_block(fig, "Supported Findings", _format_support_lines(payload), x=0.08, y=0.40, width=46, max_lines=None, font_size=9.0)
    _draw_text_block(fig, "Uncertain Findings", _format_uncertain_lines(payload), x=0.54, y=0.40, width=46, max_lines=None, font_size=9.0)
    _draw_text_block(fig, "Top Swin Probabilities", _format_top_probabilities(payload["case_packet"]).splitlines(), x=0.08, y=0.20, width=92, max_lines=None, font_size=9.0)
    pdf.savefig(fig)
    plt.close(fig)
...
def export_pdf(report_path: Path, output_path: Path) -> None:
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    _save_annotated_png(payload, output_path.with_name(f"{output_path.stem}_annotated.png"))
