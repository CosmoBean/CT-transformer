...
Chest radiography accounts for roughly 40\% of all diagnostic imaging worldwide, and the resulting volume makes it a natural target for machine learning assistance. However, the clinical value of an automated system depends less on its classification accuracy in isolation than on whether it can be integrated into the way radiologists actually work. A radiologist reading a chest X-ray performs several interleaved tasks: screening for abnormalities, localizing the anatomical basis for each finding, assessing how confident the evidence is, and composing a structured impression that another clinician can act on. Most existing automated systems address only the first task --- image-level classification --- and stop at a probability vector that is opaque to the downstream user.
...
The most practically important result is the from-scratch CNN: with only 1.31M parameters, it reaches 0.9314 macro AUC-ROC. This matters for the pipeline because it demonstrates that the classification stage does not strictly require a large transformer. In a resource-constrained deployment --- for instance, a clinic without GPU infrastructure for a 87M-parameter model --- the lightweight CNN could serve as the classifier while the localization and reporting stages remain unchanged. This is a direct benefit of the modular pipeline design.
...
The best YOLOv8m run reached 0.3666 mAP@0.5; a larger YOLOv8l variant did not materially improve this. The modest detection mAP is expected given the difficulty of the task --- VinBigData bounding boxes include diffuse abnormalities like lung opacity and ILD, which lack the well-defined spatial boundaries that object detectors are designed for. However, detection mAP alone understates the detector's value in the pipeline. The purpose of YOLO is not to compete with Swin on classification but to provide a second, spatially grounded evidence channel. When the classifier flags cardiomegaly and the detector draws a bounding box over the cardiac silhouette, the reporting layer can state that the finding has both image-level and localized support. When only one source provides evidence, the report marks the finding as uncertain. This agreement-disagreement signal is the primary way localization contributes to clinical explainability.
...
The stronger contribution of the reporting stage is qualitative rather than quantitative. Rather than a single label vector, the clinician receives a structured summary that explicitly states which findings are supported by both models, which are uncertain, and where the two sources disagree --- information that is invisible in a raw probability output.
...
This has practical implications for how such a system would be used. A clinician interacting with the pipeline sees three layers of information: which abnormalities the classifier predicts (with probabilities), where the detector places supporting bounding boxes (if any), and a structured summary that distinguishes well-supported findings from uncertain ones. If the classifier and detector disagree on a finding, the report says so. If the classifier is confident but the detector found no spatial evidence, the report marks the finding as uncertain. This transparency is fundamentally different from receiving a single probability vector, which provides no insight into why a prediction was made or how confident different evidence sources are.
...
\section{Conclusion}
This work presents a multi-stage chest X-ray decision-support pipeline designed to bridge the gap between model output and clinical usability. The pipeline mirrors the cognitive workflow a radiologist follows --- classification, localization, and structured reporting --- and keeps each stage modular and auditable. Swin provides the strongest classification performance, a lightweight CNN demonstrates that the architecture remains viable under resource constraints, YOLO contributes spatial evidence that strengthens explainability for focal findings, and the agentic reporting layer transforms raw model outputs into a structured summary that a clinician can review, question, and act on. The system is best understood not as a diagnostic model but as a first-pass assistant: a tool that organizes evidence and makes uncertainty explicit so that the clinician's review starts from a structured foundation rather than a raw probability vector.
