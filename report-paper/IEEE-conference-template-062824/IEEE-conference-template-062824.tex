...
\caption{Ground-truth-versus-YOLO comparison examples. In each row, the left panel shows merged radiologist ground-truth boxes and the right panel shows YOLO predictions. The first row is a single-label case where the YOLO box matches the radiologist localization for \textit{Aortic enlargement}. The second row is a multi-label case where YOLO recovers \textit{Aortic enlargement}, \textit{Cardiomegaly}, and \textit{Nodule/Mass}, but misses the more diffuse \textit{Lung Opacity} finding.}
