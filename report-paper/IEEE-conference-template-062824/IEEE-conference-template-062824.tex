...
This work does not claim to be the first to combine imaging models with language generation. The intended contribution is pragmatic: rather than train a new end-to-end report generator, the pipeline starts from a bounded VinBigData subset with image-level labels and localization annotations, then assembles a reproducible decision-support workflow that keeps the classifier, detector, and reporting stages explicitly separable and auditable.
...
Ground truth, Swin, YOLO, and the final report all indicated \textit{No finding}. This is the simplest workflow setting: the system produces a conservative normal summary without forcing unnecessary explanation.

\medskip
\noindent\textbf{Clean cardiomediastinal case (`a3d5137d\ldots`).}
The radiologist labels were \textit{Aortic enlargement} and \textit{Cardiomegaly}. Both Swin and YOLO agreed exactly, making the final report straightforward to support with both image-level and localized evidence.

\medskip
\noindent\textbf{Multi-finding case (`127f6676\ldots`).}
The radiologist labels were \textit{Aortic enlargement}, \textit{Cardiomegaly}, \textit{Lung Opacity}, and \textit{Nodule/Mass}. Swin recovered all four findings, while YOLO localized the three more focal patterns and missed the more diffuse \textit{Lung Opacity}. This is a representative example of the intended division of labor: the classifier captures the broader abnormal picture, while the detector contributes localization support where spatial evidence is most meaningful.
