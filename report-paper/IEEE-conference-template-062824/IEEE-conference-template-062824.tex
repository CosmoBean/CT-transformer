...
Chest X-ray AI has already been studied extensively across classification, localization, and report generation. In image-level classification, CheXNet demonstrated the practical value of deep convolutional transfer learning for chest radiograph interpretation \cite{chexnet}. On the VinDr side, Pham et al. introduced an explainable VinDr-CXR system that explicitly combined multi-label classification with lesion localization and showed that such assistance could improve interobserver agreement among readers \cite{pham2022vindr}.

Automated chest X-ray report generation has also been studied before the recent wave of multimodal LLMs. Fine-grained label learning \cite{syedamahmood2020finegrained}, contrastive attention \cite{liu2021contrastive}, and variational topic inference \cite{najdenkoska2022uncertainty} all aim to improve how visual findings are translated into clinically plausible report text. More recent multimodal approaches such as CXR-LLaVA \cite{lee2023cxrllava} and reader studies on GPT-4 impression generation \cite{ziegelmayer2023gpt4} show that large vision-language models can now produce readable chest X-ray summaries, but they are usually trained and evaluated on large report corpora such as MIMIC-CXR rather than on a label-and-box dataset like the retained VinBigData subset used here.
...
The author thanks the VinBigData dataset providers for releasing the chest X-ray data through an open Kaggle challenge, which made this project possible. The author also thanks Dr.\ Newell Washburn for course guidance and Tushar Nayak for candid reviews and feedback that helped shape the direction of the project. Compute support from the Pittsburgh Supercomputing Center is also gratefully acknowledged.
